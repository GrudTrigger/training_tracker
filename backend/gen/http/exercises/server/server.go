...
// Server lists the exercises service endpoint HTTP handlers.
type Server struct {
	Mounts []*MountPoint
	Create http.Handler
	All    http.Handler
...
// New instantiates HTTP handlers for all the exercises service endpoints using
...
func (s *Server) Service() string { return "exercises" }
...
func (s *Server) MethodNames() []string { return exercises.MethodNames[:] }

// Mount configures the mux to serve the exercises endpoints.
...
// MountCreateHandler configures the mux to serve the "exercises" service
// "create" endpoint.
...
// calls the "exercises" service "create" endpoint.
func NewCreateHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ctx = context.WithValue(ctx, goa.ServiceKey, "exercises")
...
// MountAllHandler configures the mux to serve the "exercises" service "all"
// endpoint.
...
// the "exercises" service "all" endpoint.
func NewAllHandler(
	endpoint goa.Endpoint,
	mux goahttp.Muxer,
	decoder func(*http.Request) goahttp.Decoder,
...
// MountUpdateHandler configures the mux to serve the "exercises" service
// "update" endpoint.
func MountUpdateHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("PUT", "/exercise/update", f)
}

// NewUpdateHandler creates a HTTP handler which loads the HTTP request and
// calls the "exercises" service "update" endpoint.
...
// MountDeleteHandler configures the mux to serve the "exercises" service
// "delete" endpoint.
func MountDeleteHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("DELETE", "/exercise/{exerciseId}", f)
}

// NewDeleteHandler creates a HTTP handler which loads the HTTP request and
// calls the "exercises" service "delete" endpoint.
